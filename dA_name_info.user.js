...
var links;
var breit=document.body.offsetWidth;
var naminf;
function hoverset(){
	if(document.getElementById("dA_nameinfo")==null){
		naminf=document.createElement("div");
		naminf.style.zIndex="999";
		naminf.id="dA_nameinfo";
		naminf.setAttribute("dA_nameinfo","false");
		naminf.innerHTML="###0###<br>###1###<br>###2###<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://###3###.deviantart.com/gallery/'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://###3###.deviantart.com/favourites/'>Favourites</a>";
		document.body.appendChild(naminf);
		naminf.addEventListener("mouseenter",hov,false);
		naminf.addEventListener("mouseleave",out,false);
		
		
		var sty=document.createElement("style");
		sty.innerHTML ="#dA_nameinfo{text-decoration:none;border:none!important;line-height:12pt!important;z-index: 1;text-align:left;margin-top: -15px;background-repeat: no-repeat!important;display: none;font: 8pt verdana;color:#304030;padding-left: 23px;padding-right: 10px;padding-top: 15px;position: absolute;width: 180px;background-size:105% 110%;}"+
		  // "a.u:hover #dA_nameinfo {display:inline;}"+
		 "#dA_nameinfo div.dA_nameinfo_links{font: 10pt/20px verdana;color:#304030;font-weight:bold;padding-right:5px;display:inline;} "+
		 "#dA_nameinfo div.dA_nameinfo_links div{background-image: url('http://st.deviantart.net/minish/main/more.png?7')!important;float: left;height: 20px;width: 20px;}"+
		 "#dA_nameinfo div.dA_nameinfo_links a{font: 10pt/20px  verdana;color:#6f756e!important;float:left;}"+
		 "#dA_nameinfo div.dA_nameinfo_i2{background-position:-840px 0!important;}"+
		 "#dA_nameinfo hr {margin: 3px 0px;}";
		sty.type="text/css";
		sty.id='nameinfostyle';
		document.head.appendChild(sty);
	}
	for(var i=0;i<links.length;i++){
		links[i].addEventListener("mousemove",function(arg){
			if(arg.pageX>breit-naminf.offsetWidth){
				naminf.style.backgroundImage="url('"+backaltimg+"')";
				naminf.style.left=(arg.pageX-naminf.offsetWidth)+"px";
			}else{
				naminf.style.backgroundImage="url('"+backimg+"')";
				naminf.style.left=(arg.pageX+1)+"px";
			}
			naminf.style.top=(arg.pageY)+"px";
		},false);
		links[i].addEventListener("mouseenter",hov,false);
		links[i].addEventListener("mouseleave",out,false);
	}
...
function hov(arg){
        ...
        if (this.getAttribute("title")!= false) {
            this.setAttribute("deaktitle",this.getAttribute("title"));
            this.setAttribute("title","");
        }
		this.setAttribute("showit","true");		
		if(this!=naminf){
			if(naminf.style.display=="block")return true;
			console.log(naminf,arg);
		...
		if(this.getAttribute("href")!="" && this.tagName=="A"){
			user=this.getAttribute("href").match(/http:\/\/(.*)\.deviantart/i);
			if(user)user=user[1].toLowerCase();
		}else if(this.hasAttribute("class")&&this.tagName=="DD"){
			user=this.getAttribute("class").match(/dAmnChatMember un-(.*?) ffc/i)[1];
		}		
        if(this.hasAttribute("href")&& ((this.getAttribute("href").search(/^http:\/\/[^.]*?\.deviantart.com\/art\//)>-1)||(this.getAttribute("href").search(/^http:\/\/fav\.me\/\w+$/)>-1))){
            aktthumb=true;
			naminf.style.width="300px";
			naminf.style.paddingLeft="30px";
			
            thumb=this.getAttribute("href").match(/^(?:http:\/\/[^.]*?\.deviantart.com\/art\/.*?(\d+).*?)|(?:http:\/\/fav\.me\/(\w+))$/)[1];
            if(typeof thumbdata[thumb]=="undefined"){
                naminf.innerHTML=("Please Wait!<br>Deviation-data are being fetched<br>...<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://www.deviantart.com'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://www.deviantart.com'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
			    getdevdata(this);
                return false;
            }
            var arg=thumbdata[thumb];	            
     		naminf.innerHTML=naminf.innerHTML.replace("###0###",thumbdata[thumb][0]).replace("###1###",thumbdata[thumb][1]).replace("###2###",thumbdata[thumb][2]).replace(/###3###/g,thumbdata[thumb][3]).replace("###4###",thumbdata[thumb][4]);
            this.setAttribute("dA_nameinfo","true");
        }else if(!this.hasAttribute("href")&&aktthumb){
            var arg=thumbdata[thumb];	            
     		if(typeof thumbdata[thumb]=="undefined"){
                naminf.innerHTML=("Please Wait!<br>Deviation-data are being fetched<br>...<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://www.deviantart.com'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://www.deviantart.com'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
                getdevdata(this);
            ...
            aktthumb=false;
			naminf.style.width="";
			naminf.style.paddingLeft="";
            if(typeof userdata[user]=="undefined"){
                naminf.innerHTML=("Please Wait!<br>User-data are being fetched<br>...<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://www.deviantart.com'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://www.deviantart.com'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
                getdata(this);
                return false;
            }
            ...
            naminf.innerHTML=naminf.innerHTML.replace("###0###",arg[1]).replace("###1###",arg[2]).replace("###2###",arg[0]).replace(/###3###/g,user).replace("###4###",formerly);
            this.setAttribute("dA_nameinfo","true");
        }		

}
function out(){
	var deakt=document.querySelector("[deaktitle]");
	if(deakt!=null){
		deakt.setAttribute("title",this.getAttribute("deaktitle"));
		deakt.removeAttribute("deaktitle");
	}
	this.removeAttribute("showit");
	if(document.querySelector("[showit]")==null){
		naminf.style.display="none";
		naminf.innerHTML=("###0###<br>###1###<br>###2######4###<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://###3###.deviantart.com/gallery/'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://###3###.deviantart.com/favourites/'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
	}
}	
	
function getdevdata(el){
	el.setAttribute("dA_nameinfo","false");
	...
	GM_xmlhttpRequest({
		...
		onload: function (responseDetails) {
            var rex=/<dt>Favourites<\/dt><dd>(\d+)[\s\S]*?<\/dd>[\s\S]*<dt>Comments<\/dt><dd>(\d+)[\s\S]*?<\/dd></;
            var rex2=/<meta name="title" content="(.*?) on deviantART"/;
            var rex3=/<a.*?target="_self".*?href="http:\/\/.*?.deviantart.com/;
            ...
            if(ergs!=null){
                thumbdata[thumb]=[ergs2[1],ergs3[1],ergs[1]+" comments and "+ergs[2]+" favourites",ergs2[1].match(/by .(.+?)$/)[1],ergs4];
                naminf.innerHTML=("###0###<br>###1###<br>###2######4###<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://###3###.deviantart.com/gallery/'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://###3###.deviantart.com/favourites/'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
				naminf.innerHTML=naminf.innerHTML.replace("###0###",thumbdata[thumb][0]).replace("###1###",thumbdata[thumb][1]).replace("###2###",thumbdata[thumb][2]).replace(/###3###/g,thumbdata[thumb][3]).replace("###4###",thumbdata[thumb][4]);
            }else{
...
function getdata(el){
	el.setAttribute("dA_nameinfo","false");
	if(!el.hasAttribute("href"))return false;
	if(el.parentNode.innerHTML.indexOf("#")!=-1){return true;}
	...
	GM_xmlhttpRequest({
		...
		onload: function (responseDetails) {
				var rex = /<span style="display:table-cell;\*display:inline-block;padding-right:16px">[\s\S]*?<strong class="f">([\s\S]*?)<\/strong>[\s\S]*?<strong class="f">[\s\S]*?<\/strong>[\s\S]*?<\/span>[\s\S]*?<span style="display:table-cell;\*display:inline-block"><strong class="f realname">([\s\S]*?)<\/strong>([\s\S]*?)<\/span>/gi;
				var ergs=rex.exec(responseDetails.responseText);
				// rex=/<a class="u" href="http:\/\/(.*?).deviantart.com">.*?<\/a>/i;
				rex=/:\/\/(.*?)\.deviantart\.com/i;
				var rex2 =/<div class="grurename"><span>Formerly <strong>(.*?)<\/strong><\/span><\/div>/i;
				var ergs2=rex2.exec(responseDetails.responseText);
				if(ergs2!=null){
					var pins=document.querySelector("a[href='"+el.getAttribute("href")+"']").parentNode;
					var pel=document.createElement("span");
					pel.className='alterego';
					pel.style.cssText="margin:0px -10px 0px 10px;font-size: 8.25pt;color: #778584; white-space: nowrap;";
					pel.innerHTML="Formerly "+ergs2[1]+"<span style='border-left: 1px dotted #778584; margin:0px 0px 0px 5px'>&nbsp;</span>";
					pins.parentNode.insertBefore(pel,pins.nextSibling);
				}
				if(ergs!=null){
					var thisuser=rex.exec(responseDetails.finalUrl)[1].toLowerCase();
					userdata[thisuser]=ergs[1]+"|"+ergs[2]+"|"+ergs[3]+"|";
					if(ergs2!=null)userdata[thisuser]+=ergs2[1];
					users.push(thisuser);
					naminf.innerHTML=("###0###<br>###1###<br>###2######4###<hr><div class='dA_nameinfo_links'><div class='dA_nameinfo_i1'></div><a href='http://###3###.deviantart.com/gallery/'>Gallery</a><div class='dA_nameinfo_i2'></div><a href='http://###3###.deviantart.com/favourites/'>Favourites</a><br style='clear:both;margin-bottom:7px;'/></div>");
					if(ergs[2].length>30){ergs[2]=ergs[2].substr(0,25)+" ...";}
					var formerly="";
					if(ergs2!=null&&ergs2[1]!="")formerly="<br>Former "+ergs2[1];
					naminf.innerHTML=naminf.innerHTML.replace("###0###",ergs[2]).replace("###1###",ergs[3]).replace("###2###",ergs[1]).replace(/###3###/g,thisuser).replace("###4###",formerly);
					el.setAttribute("dA_nameinfo","true");
				}else{
					console.log("Error: can't fetch data!");
					console.log(responseDetails.responseText);
				}
...
function urlpruf(){	
	links=[];
	var plinks=document.querySelectorAll("a[href]:not([da_nameinfo]),dd:not([da_nameinfo])");
		for(var i=0;i<plinks.length;i++){
			if(plinks[i].className.search(/dAmnChatMember/)!=-1){links.push(plinks[i]);continue;}
			if(!plinks[i].hasAttribute("href"))continue;
			if((plinks[i].getAttribute("href").search(/^http:\/\/[^.]*?\.deviantart.com\/$/)>-1)||
				(plinks[i].getAttribute("href").search(/^http:\/\/fav\.me\/\w+$/)>-1)||
				(plinks[i].getAttribute("href").search(/^http:\/\/[^.]*?\.deviantart.com\/art\//)>-1))
					links.push(plinks[i]);	
		}
